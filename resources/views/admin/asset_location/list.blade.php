@extends('layouts.app')
    ...
    <main class="app-main"> 
        <div class="app-content-header"> 
            <div class="container-fluid"> 
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Ubicaciones de Activos</h3>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Listado de Ubicaciones de Activos</h3>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addBrandModalLabel">Agregar Ubicación</h5>
                ...
                <div class="modal-body">
                    <form id="addBrandForm">
                        {{ @csrf_field() }}
                        <div class="mb-3">
                            <label for="brandName" class="form-label"><b>Nombre de Ubicación</b></label>
                            <input type="text" class="form-control" id="name" name="name" required>
                        </div>
                        <div class="mb-3">
                            <label for="brandName" class="form-label"><b>Descripción</b></label>
                            <input type="text" class="form-control" id="description" name="description" required>
                        </div>
                        <div class="mb-3">
                            <label for="brandName" class="form-label"><b>Dirección</b></label>
                            <input type="text" class="form-control" id="address" name="address" required>
                        </div>
                        <div class="mb-3">
                            <label for="brandName" class="form-label"><b>Responsable</b></label>
                            <input type="text" class="form-control" id="responsible_person" name="responsible_person" required>
                        </div>
                        <div class="mb-3">
                            <label for="brandName" class="form-label"><b>Estado</b></label>
   ...
   <script type="text/javascript">
        $(document).ready(function(){
            ...
            function fechtBrand(){
                $.ajax({
                    url: "{{url('admin/asset_location/data')}}",
                    type: 'GET',
                    success: function(response){
                        ...
                        $.each(response, function(index,brand){
                            ...
                            let status = brand.status === 1 ? 'Activo' : 'Inactivo';
                            tableBody += `<tr>
                                        <td>${index + 1}</td>
                                        <td>${brand.name}</td>
                                        <td>${brand.description}</td>
                                        <td>${brand.address}</td>
                                        <td>${brand.responsible_person}</td>
                                        <td>${status}</td>
                                        <td>${createdAt}</td>
                                        <td>${updatedAt}</td>
                                        <td>
                    ...
                    error: function(xhr, status, error){
                        console.error('Error al leer las ubicaciones: ', error);
                    }
            ...
            function handleEdit(e) {
                ...
                let brandId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/asset_location/edit') }}/" + brandId,
                  type: 'GET', success: function(brand)
                   { // Establecer los valores en los campos del modal
                     ...
                     $('#responsible_person').val(brand.responsible_person);
                      ...
                      $('#addBrandModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addBrandForm').off('submit').on('submit', function(e) {
                         ...
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/asset_location/update') }}/" + brandId, 
                          type: 'POST', 
                          data: formData,
                           success: function(response)
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/asset_location/delete') }}/" + brandId,
                type: 'DELETE',
                data: {
                    _token: "{{ csrf_token() }}"
                },
...
$(document).ready(function() {
   ...
   $(document).ready(function(){
    $('#addBrandForm').on('submit', function (e){
        ...
        $.ajax({
            url: "{{ url('admin/asset_location/store') }}",
            type: 'POST',
            data: formData,
            success: function(response){
                $('#addBrandModal').modal('hide');
