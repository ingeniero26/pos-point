@extends('layouts.app')
    ...
    <div class="modal fade" id="addSupplierModal" aria-labelledby="addSupplierModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                ...
                <div class="modal-body">
                    ...
                    <form id="addSupplierForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="identification_number" class="form-label"><b>Razon Social</b></label>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="contact_name" class="form-label"><b>Nombre de Contacto</b></label>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control" id="email" name="email" onblur="duplicateEmail(this)">
                                </div>
                            </div>
                        </div>
                        <div class="row">
   ...
   <script type="text/javascript">
    ...
    if (email) {
        $.ajax({
            url: '{{ url('admin/supplier/check-email') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                email: email
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            $.each(response, function(index, supplier){
                let createdAt = dayjs(supplier.created_at).format('DD/MM/YYYY h:mm A');
                let updatedAt = dayjs(supplier.updated_at).format('DD/MM/YYYY h:mm A');
                let statusText = supplier.status == 1 ? 'Inactivo' : 'Activo';
              
                let toggleStatusText = supplier.status == 1 ? 'Activar' : 'Desactivar';
                let toggleIcon = supplier.status == 1 ? 'fa-toggle-on' : 'fa-toggle-off';

                tableBody += `<tr>
                                <td>${index + 1}</td>
                               
                                <td>${supplier.identification_type ? supplier.identification_type.identification_name : 'N/A'}</td>
                                <td>${supplier.identification_number}</td>
                                <td>${supplier.company_name}</td>
                                <td>${supplier.contact_name}</td>
                                <td>${supplier.contact_last_name}</td>
                                <td>${supplier.departments ? supplier.departments.name_department : 'N/A'}</td>
                                <td>${supplier.cities ? supplier.cities.city_name : 'N/A'}</td>
                                ...
                                <td>${createdAt}</td>
                                <td>
                                    <button class="btn btn-warning btn-sm edit-btn" data-id="${supplier.id}"><i class="fa-solid fa-pen"></i></button>
                                    <button class="btn btn-danger btn-sm delete-btn" data-id="${supplier.id}"><i class="fa-solid fa-trash"></i></button>
                                    <button class="btn btn-secondary btn-sm toggle-status-btn" data-id="${supplier.id}" data-status="${supplier.status}"><i class="fa ${toggleIcon}"></i> ${toggleStatusText}</button>
                                    <button class="btn btn-info btn-sm view" data-id="${supplier.id}"><i class="fa-regular fa-eye"></i></button>
                                </td>
                             </tr>`;
            });
...
$(document).ready(function() {
    ...
    $(' #filterStatus').change(function() {
        ...
        function handleToggleStatus(e) {
            ...
            $.ajax({
                url: "{{ url('admin/supplier/toggle-status') }}/" + supplierId,
                type: 'PUT',
                data: {
                    _token: "{{ csrf_token() }}",
                    status: newStatus
                ...
                success: function(response) {
                    ...
                    fechtSupplier(); // Refrescar la lista de ciudades
                },
                error: function(xhr, status, error) {
        ...
        function handleView(e)
        {
            ...
            window.location.href = "{{ url('admin/supplier') }}/" + supplierId;
        }
    ...
    $.ajax({
        url: "{{ url('admin/supplier/edit') }}/" + supplierId,
        type: 'GET',
        success: function(supplier) { 
            // Establecer los valores en los campos del modal
          
            $('#identification_type_id').val(supplier.identification_type_id);
            $('#identification_number').val(supplier.identification_number);
            $
            $('#company_name').val(supplier.company_name);
            $('#contact_name').val(supplier.contact_name);
            $('#contact_last_name').val(supplier.contact_last_name);
            $('#type_person').val(supplier.type_person);
            $('#tax_liability').val(supplier.tax_liability);
            $('#department_id').val(supplier.department_id);
            $('#city_id').val(supplier.city_id);
           ...
           $('#email').val(supplier.email);
            ...
            $('#addSupplierForm').off('submit').on('submit', function(e) {
                ...
                $.ajax({ 
                    url: "{{ url('admin/supplier/update') }}/" + supplierId,
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        $('#addSupplierModal').modal('hide');
                        fechtSupplier(); // Actualiza la lista de productos
...
function handleDelete(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('admin/supplier/delete') }}/" + supplierId,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
