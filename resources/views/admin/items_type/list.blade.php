        ...
        error: function (xhr, status, error) {
            ...
            function handleEdit(e) {
                ...
                let items_typeId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/items_type/edit') }}/" + items_typeId,
                  type: 'GET',
                      ...
                      $('#addItemTypeModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addItemTypeForm').off('submit').on('submit', function(e) {
                         ...
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/items_type/update') }}/" + items_typeId, 
                          type: 'POST', 
                          data: formData,
                           success: function(response)
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('admin/items_type/delete') }}/" + items_typeId,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
...
$(document).ready(function() {
   ...
   $(document).ready(function(){
    $('#addItemTypeForm').on('submit', function (e){
        ...
        $.ajax({
            url: "{{ url('admin/items_type/store') }}",
            type: 'POST',
            data: formData,
            success: function(response){
                $('#addItemTypeModal').modal('hide');
