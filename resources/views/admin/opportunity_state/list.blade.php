@extends('layouts.app')
    ...
    <main class="app-main"> 
        <div class="app-content-header"> 
            <div class="container-fluid"> 
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Estados de las Oportunidades</h3>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Listado de Estados de las Oportunidades</h3>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addItemTypeModalLabel">Agregar</h5>
                ...
                <div class="modal-body">
                    <form id="addItemTypeForm">
                        ...
                        <div class="mb-3">
                            <label for="taxDescription" class="form-label"><b>Descripción</b></label>
   ...
   <script type="text/javascript">
    ...
    $.ajax({
        url: "{{ url('admin/opportunity_state/data') }}",
        type: 'GET',
        success: function (response) {
            let tableBody = '';

            $.each(response, function (index, opportunityState) {
                // Verificar si is_delete = 1 (opcional, si el backend ya filtra)
                if (opportunityState.is_delete === 0) {
                    let createdAt = dayjs(opportunityState.created_at).format('DD/MM/YYYY h:mm A');
                    let updatedAt = dayjs(opportunityState.updated_at).format('DD/MM/YYYY h:mm A');
                    tableBody += `<tr>
                                <td>${index + 1}</td>
                                <td>${opportunityState.name}</td>
                                <td>${opportunityState.description}</td>
                                <td>${createdAt}</td>
                                <td>${updatedAt}</td>
                                <td>
                                    <button class="btn btn-warning btn-sm edit-btn" data-id="${opportunityState.id}"><i class="fa-solid fa-pen"></i></button>
                                    <button class="btn btn-danger btn-sm delete-btn" data-id="${opportunityState.id}"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>`;
                }
            });
        ...
        error: function (xhr, status, error) {
            ...
            function handleEdit(e) {
                ...
                let items_typeId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/opportunity_state/edit') }}/" + items_typeId,
                  type: 'GET',
                      ...
                      $('#addItemTypeModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addItemTypeForm').off('submit').on('submit', function(e) {
                         ...
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/opportunity_state/update') }}/" + items_typeId, 
                          type: 'POST', 
                          data: formData,
                           success: function(response)
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('admin/opportunity_state/delete') }}/" + items_typeId,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
...
$(document).ready(function() {
   ...
   $(document).ready(function(){
    $('#addItemTypeForm').on('submit', function (e){
        ...
        $.ajax({
            url: "{{ url('admin/opportunity_state/store') }}",
            type: 'POST',
            data: formData,
            success: function(response){
                $('#addItemTypeModal').modal('hide');
