@extends('layouts.app')
    ...
    <main class="app-main"> 
        <div class="app-content-header"> 
            <div class="container-fluid"> 
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Tipo Devoluciones Compra</h3>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Listado de Tipos de Devoluciones</h3>
   ...
   <script type="text/javascript">
        $(document).ready(function(){
            ...
            function fechtBrand(){
                $.ajax({
                    url: "{{url('admin/return_reasons/data')}}",
                    type: 'GET',
                    success: function(response){
                        let tableBody='';
                        $.each(response, function(index,returnReason){
                            let createdAt = dayjs(returnReason.created_at).format('DD/MM/YYYY h:mm A');
                            let updatedAt = dayjs(returnReason.updated_at).format('DD/MM/YYYY h:mm A');
                            tableBody += `<tr>
                                        <td>${index + 1}</td>
                                        <td>${returnReason.name}</td>
                                        <td>${returnReason.description}</td>
                                        <td>${returnReason.status ? 'Activo' : 'Inactivo'}</td>
                                       
                                        <td>${createdAt}</td>
                                        <td>${updatedAt}</td>
                                        <td>
                                            <button class="btn btn-warning btn-sm edit-btn" data-id="${returnReason.id}"><i class="fa-solid fa-pen"></i></button>
                                            <button class="btn btn-danger btn-sm delete-btn" data-id="${returnReason.id}"><i class="fa-solid fa-trash"></i></button>
                                        </td>
            ...
            function handleEdit(e) {
                ...
                let brandId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/return_reasons/edit') }}/" + brandId,
                      ...
                      $('#addBrandModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addBrandForm').off('submit').on('submit', function(e) {
                         ...
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/return_reasons/update') }}/" + brandId, 
                          type: 'POST', 
                          data: formData,
                            headers: {
        ...
        function handleDelete(e) {
             ...
             let brandId = $(this).data('id');
                ...
                $.ajax({
                    url: "{{ url('admin/return_reasons/delete') }}/" + brandId,
                    type: 'DELETE',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
...
$(document).ready(function() {
   ...
   $(document).ready(function(){
    $('#addBrandForm').on('submit', function (e){
        ...
        $.ajax({
            url: "{{ url('admin/return_reasons/store') }}",
            type: 'POST',
            data: formData,
            success: function(response){
                $('#addBrandModal').modal('hide');
