        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addWarehouseModalLabel">Agregar Bodega</h5>
                ...
                <div class="modal-body">
                    <form id="addWarehouseForm">
                        {{ @csrf_field() }}
                        <div class="mb-3">
                            <label for="warehouseName" class="form-label"><b>Nombre de Categoria</b></label>
   ...
   <script type="text/javascript">
        $(document).ready(function(){
            ...
            function fectWarehouse(){
                $.ajax({
                    url: "{{url('admin/warehouse/data')}}",
                    type: 'GET',
                    success: function(response){
                        let tableBody='';
                        $.each(response, function(index,warehouse){
                            let createdAt = dayjs(warehouse.created_at).format('DD/MM/YYYY h:mm A');
                            let updatedAt = dayjs(warehouse.updated_at).format('DD/MM/YYYY h:mm A');
                            tableBody += `<tr>
                                        <td>${index + 1}</td>
                                        ...
                                        <td>${updatedAt}</td>
                                        <td>
                                            <button class="btn btn-warning btn-sm edit-btn" data-id="${warehouse.id}"><i class="fa-solid fa-pen"></i></button>
                                            <button class="btn btn-danger btn-sm delete-btn" data-id="${warehouse.id}"><i class="fa-solid fa-trash"></i></button>
                                        </td>
            ...
            function handleEdit(e) {
                ...
                let warehouseId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/warehouse/edit') }}/" + warehouseId,
                  type: 'GET', success: function(warehouse)
                      ...
                      $('#addWarehouseModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addWarehouseForm').off('submit').on('submit', function(e) {
                         ...
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/warehouse/update') }}/" + warehouseId, 
                          type: 'POST', 
                          data: formData,
                           success: function(response)
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esta acción eliminará permanentemente la bodega.",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ url('admin/warehouse/delete') }}/" + warehouseId,
                type: 'DELETE',
                data: {
                    _token: "{{ csrf_token() }}"
                },
