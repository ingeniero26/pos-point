@extends('layouts.app')
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <h3 class="card-title">Listado de Tipo Obligacion</h3>
                                <div class="card-tools">
                                    <ul class="pagination pagination-sm float-end">
                                        <a href="" class="btn btn-sm btn-primary"
                                         data-bs-toggle="modal" data-bs-target="#addCustomerModal">
                                         Agregar Tipo Obligación
                                        </a>
                                    </ul>
                                </div>
                            </div>
   ...
   <script type="text/javascript">
    ...
    $.ajax({
        url: "{{route('type_liability.fetch')}}",
        type: 'GET',
        data: {
...
$(document).ready(function() {
    ...
    $(' #filterStatus').change(function() {
        ...
        function handleToggleStatus(e) {
            ...
            $.ajax({
                url: "{{ url('admin/type_liability/toggle-status') }}/" + customerId,
                type: 'PUT',
                data: {
                    _token: "{{ csrf_token() }}",
                    status: newStatus
        ...
        function handleView(e)
        {
            ...
            window.location.href = "{{ url('admin/type_liability') }}/" + customerId;
        }
    ...
    $.ajax({
        url: "{{ url('admin/type_liability/edit') }}/" + customerId,
        type: 'GET',
        success: function(customer) { 
            ...
            $('#liability_name').val(customer.liability_name);    
            ...
            $('#addCustomerForm').off('submit').on('submit', function(e) {
                ...
                $.ajax({ 
                    url: "{{ url('admin/type_liability/update') }}/" + customerId,
                    type: 'POST',
                    data: formData,
                    success: function(response) {
...
function handleDelete(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('admin/type_liability/delete') }}/" + customerId,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#addCustomerForm').on('submit', function (e){
            ...
            $.ajax({
                url: "{{route('type_liability.store')}}",
                type: 'POST',
                data: formData,
                success: function(response){
                    $('#addCustomerModal').modal('hide');
