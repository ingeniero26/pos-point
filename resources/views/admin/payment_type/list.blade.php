@extends('layouts.app')
                                ...
                                <div class="card-tools">
                                    <ul class="pagination pagination-sm float-end">
                                        <a href="" class="btn btn-sm btn-primary"
                                         data-bs-toggle="modal" data-bs-target="#addFormPaymentModal">
    ...
    <div class="modal fade" id="addFormPaymentModal" tabindex="-1" aria-labelledby="addFormPaymentModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addFormPaymentModalLabel">Agregar Tipo de Pago</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addFormPaymentForm">
                        {{ @csrf_field() }}
                        ...
                        {{-- <div class="mb-3">
                            <label for="days" class="form-label"><b>Días de Vigencia</b></label>
                            <input type="number" class="form-control" id="days" name="days" required>
                            <small id="daysHelp" class="form-text">Los días de vigencia deben ser un número entero.</small>
                        </div> --}}
   ...
   <script type="text/javascript">
    ...
    $.ajax({
        url: "{{ url('admin/payment_type/data') }}",
        type: 'GET',
        success: function (response) {
            ...
            $.each(response, function (index, getDocumentTypes) {
                ...
                if (getDocumentTypes.is_delete === 0) {
                    ...
                    tableBody += `<tr>
                                <td>${index + 1}</td>
                                <td>${getDocumentTypes.payment_type}</td>
                                <td>${getDocumentTypes.description}</td>
        ...
        error: function (xhr, status, error) {
            ...
            function handleEdit(e) {
                ...
                let account_document_sourceId = $(this).data('id');
                 $.ajax({
                     url: "{{ url('admin/payment_type/edit') }}/" + account_document_sourceId,
                  type: 'GET',
                   ...
                   { // Establecer los valores en los campos del modal
                     ...
                     $('#code').val(bank.code);
                      $('#name').val(bank.name); 
                      $('#addFormPaymentModal').modal('show'); // Manejar el envío del formulario de edición
                       $('#addFormPaymentForm').off('submit').on('submit', function(e) {
                         e.preventDefault(); // Prevenir el envío del formulario 
                         const formData = $(this).serialize();
                          $.ajax({ url: "{{ url('admin/payment_type/update') }}/" +account_document_sourceId, 
                          type: 'POST', 
                          data: formData,
                           success: function(response)
                            { $('#addFormPaymentModal').modal('hide');
                            fetchBankEntity();
                              $('#addFormPaymentForm')[0].reset(); 
                             $('.flashMessage') 
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('admin/payment_type/delete') }}/" + account_document_sourceId,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
...
$(document).ready(function() {
   ...
   <script type="text/javascript">
   $(document).ready(function(){
    $('#addFormPaymentForm').on('submit', function (e){
        e.preventDefault();
        let formData = $(this).serialize();
        $.ajax({
            url: "{{ url('admin/payment_type/store') }}",
            type: 'POST',
            data: formData,
            success: function(response){
                $('#addFormPaymentModal').modal('hide');
